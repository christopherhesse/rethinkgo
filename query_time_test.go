...
func (s *RethinkSuite) TestTime(c *test.C) {
	...
	err := Time(1986, 11, 3, 12, 30, 15, "Z").TimeFormat("").Run(session).One(&response)
...
func (s *RethinkSuite) TestISO8601(c *test.C) {
	...
	err := ISO8601("1986-11-03T08:30:00-07:00").TimeFormat("").Run(session).One(&t1)
	c.Assert(err, test.IsNil)
...
func (s *RethinkSuite) TestInTimezone(c *test.C) {
	...
	c.Assert(err, test.IsNil)
	var response []time.Time
	err = Expr(List{Now(), Now().InTimezone("-07:00")}).TimeFormat("").Run(session).One(&response)
	c.Assert(err, test.IsNil)
...
func (s *RethinkSuite) TestBetween(c *test.C) {
	...
	}).Count().TimeFormat("").Run(session).One(&response)
...
func (s *RethinkSuite) TestYear(c *test.C) {
	...
	err := Time(1986, 12, 3, 12, 30, 15, "Z").Year().TimeFormat("").Run(session).One(&response)
...
func (s *RethinkSuite) TestMonth(c *test.C) {
	...
	err := Time(1986, 12, 3, 12, 30, 15, "Z").Month().Eq(December()).TimeFormat("").Run(session).One(&response)
...
func (s *RethinkSuite) TestDay(c *test.C) {
	...
	err := Time(1986, 12, 3, 12, 30, 15, "Z").Day().Eq(Wednesday()).TimeFormat("").Run(session).One(&response)
